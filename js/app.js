...
const App = defineComponent({
  setup() {
    ...
    const setKnob = (key, v) => { knobs[key] = v; };
    ...
    const wallpaper = computed(() => window.Wallpapers.find(w => w.id === tweaks.wallpaper) || window.Wallpapers[0]);
    ...
    return () => h('div', { class: 'app-root' }, [
      ...
      h('div', { class: 'shell' }, [
        // Sidebar
        h(SS.Sidebar, {
          pages, active: activeId.value, query: query.value,
        ...
        // Right panel
        h(RightPanel, {
          page: activePage.value, knobs,
          tweaks, wallpaper: wallpaper.value, tweaksOpen: tweaksOpen.value,
          onUpdate: setKnob,
      ...
      h(TopBar, {
        tweaks, tweaksOpen: tweaksOpen.value,
      ...
      // Tweaks panel
      tweaksOpen.value ? h(TweaksPanel, { tweaks, onClose: () => tweaksOpen.value = false }) : null,
    ]);
  }
});
  ...
  setup(props, { emit }) {
    return () => {
      ...
      const dataCtx = {
        ...knobs,
        ...extras,
        updateValue: (v) => { if (page.isModel) emit('update', page.isModel, v); },
        onKey: () => {},
      };
      ...
      else if (page.custom === 'tabbar') demoBody = h(SS.TabBarDemo, { knobs, onUpdate: (k, v) => emit('update', k, v) });
      else if (typeof page.render === 'function') {
        const tpl = page.render(knobs);
        demoBody = h(SS.DynamicTemplate, { tpl: `<div class="demo-row">${tpl}</div>`, data: dataCtx });
      } else {
      ...
      return h('main', { class: 'main' }, [
        h('div', { class: 'main-inner' }, [
          ...
          h('div', { class: 'section' }, [
            ...
            h('div', {
              class: ['demo', (page.id === 'keyboard' || page.id === 'audio' || page.id === 'menu' || page.id === 'tabbar') ? 'tall' : '']
            }, demoBody),
          ]),
          (page.variants && page.variants.length) ? h('div', { class: 'section' }, [
            h('div', { class: 'section-label' }, [
  ...
  setup(props, { emit }) {
    return () => h('div', { class: 'topbar' }, [
      h(window.LiquidIconButton, {
        icon: 'palette', size: 'sm', variant: 'glass',
        label: 'Cycle wallpaper', onClick: () => emit('cycleWallpaper')
      }),
      h(window.LiquidIconButton, {
        icon: props.tweaks.theme === 'dark' ? 'sun' : 'moon',
        size: 'sm', variant: 'glass',
        label: 'Toggle theme', onClick: () => emit('theme')
      }),
      h(window.LiquidIconButton, {
        icon: 'gear', size: 'sm',
        variant: props.tweaksOpen ? 'tinted' : 'glass',
        label: 'Open tweaks', onClick: () => emit('tweaks')
      }),
    ]);
  }
});
...
const TweaksPanel = defineComponent({
  ...
  emits: ['close'],
  setup(props, { emit }) {
    return () => h('div', { class: 'tweaks-panel lq-glass-strong lq-specular' }, [
      ...
      h('div', { class: 'tweaks-body' }, [
        ...
        h('div', { class: 'tweak-group' }, [
          ...
          h('div', { class: 'tweak-wallpapers' },
            window.Wallpapers.map(w =>
              h('button', {
                class: ['tweak-wp', { active: props.tweaks.wallpaper === w.id }],
                style: w.css.replace('background:', 'background:').replace(/position: fixed.*?;/, ''),
...
function mount() {
  ...
  const comps = [
    'LiquidButton','LiquidIconButton','LiquidToggle','LiquidSegmented','LiquidSlider',
    'LiquidStepper','LiquidInput','LiquidSearchBar',
