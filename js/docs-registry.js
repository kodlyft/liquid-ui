...
window.LiquidDocs = [
  ...
  // -----------------------------------------------------------
  {
    ...
    title: 'Button',
    description:
      'The primary call-to-action. Five variants -- primary, secondary glass, destructive, tinted, and ghost -- with three sizes and optional leading/trailing icons. The signature top inner highlight + soft saturated drop shadow gives every press surface a hint of refraction.',
    component: 'LiquidButton',
    knobs: {
      label: { type: 'text', default: 'Continue' },
      variant: {
        type: 'select',
        options: ['primary', 'secondary', 'destructive', 'tinted', 'ghost'],
        default: 'primary',
      },
      ...
      icon: {
        type: 'select',
        options: ['', 'play', 'sparkles', 'plus', 'check', 'trash', 'share'],
        default: '',
      },
    ...
    render: (k) =>
      `<liquid-button variant="${k.variant}" size="${k.size}" rounded="${k.rounded}" icon="${k.icon}" icon-right="${k.iconRight}" ${k.block ? 'block' : ''} ${k.loading ? 'loading' : ''} ${k.disabled ? 'disabled' : ''}>${k.label}</liquid-button>`,
    variants: [
...
defineEmits(['click'])
...

  {
    ...
    title: 'Icon Button',
    description:
      'A circular icon-only action for toolbars, headers, and floating controls. Pairs perfectly with the larger buttons -- same glass language at a smaller surface.',
    component: 'LiquidIconButton',
    knobs: {
      icon: {
        type: 'select',
        options: ['share', 'more', 'undo', 'redo', 'search', 'plus', 'check'],
        default: 'share',
      },
      variant: {
        type: 'seg',
        options: ['glass', 'tinted', 'destructive', 'ghost'],
        default: 'glass',
      },
      size: { type: 'seg', options: ['sm', 'md', 'lg'], default: 'md' },
    ...
    render: (k) =>
      `<liquid-icon-button icon="${k.icon}" variant="${k.variant}" size="${k.size}" badge="${k.badge}" ${k.disabled ? 'disabled' : ''} label="${k.icon}"></liquid-icon-button>`,
    variants: [
...
<LiquidIconButton icon="bell" badge="3" label="Notifications" />`,
  ...
  // -----------------------------------------------------------
  {
    ...
    title: 'Toggle',
    description:
      'Binary switch with a chunky 3D thumb that springs across the track. Glow tinted to match the active color. Tap to flip; v-model binds a boolean.',
    component: 'LiquidToggle',
    knobs: {
      modelValue: { type: 'bool', default: true, label: 'value' },
      color: {
        type: 'select',
        options: ['green', 'accent', 'orange', 'red', 'purple'],
        default: 'green',
      },
    ...
    isModel: 'modelValue',
    render: (k) =>
      `<liquid-toggle :model-value="${k.modelValue}" color="${k.color}" size="${k.size}" ${k.disabled ? 'disabled' : ''} @update:model-value="updateValue"></liquid-toggle>`,
    variants: [
...
watch(() => props.indeterminate, syncIndeterminate)
...

  {
    ...
    description:
      'Pick one option from a small set. The selected indicator slides on a spring; underlying glass refracts the wallpaper behind it.',
    component: 'LiquidSegmented',
    knobs: {
      modelValue: {
        type: 'select',
        options: ['style', 'text', 'arrange'],
        default: 'text',
        label: 'value',
      },
      size: { type: 'seg', options: ['sm', 'md'], default: 'md' },
    ...
    render: (k) =>
      `<liquid-segmented :model-value="modelValue" size="${k.size}" :options="segOpts" @update:model-value="updateValue"></liquid-segmented>`,
    variants: [
      {
        label: 'Three options',
        props: {
...
/>`,
...

  {
    ...
    title: 'Slider',
    description:
      'Drag the thumb to set a value. Use the iconLeft / iconRight props for context (sun for brightness, volume for audio). Spring-thumb and soft glow on the fill.',
    ...
    isModel: 'modelValue',
    render: (k) =>
      `<liquid-slider :model-value="${k.modelValue}" color="${k.color}" icon-left="${k.iconLeft}" icon-right="${k.iconRight}" ${k.showValue ? 'show-value' : ''} ${k.disabled ? 'disabled' : ''} @update:model-value="updateValue"></liquid-slider>`,
    variants: [
...
<LiquidSlider v-model="volume" icon-left="volume" color="white" />`,
...

  {
    ...
    title: 'Input',
    description:
      'Single-line text input with optional leading icon, clearable affordance, and focus ring tinted with your accent color.',
    ...
    isModel: 'modelValue',
    render: (k) =>
      `<liquid-input :model-value="'${k.modelValue}'" placeholder="${k.placeholder}" icon="${k.icon}" size="${k.size}" ${k.clearable ? 'clearable' : ''} ${k.disabled ? 'disabled' : ''} @update:model-value="updateValue"></liquid-input>`,
    variants: [
      { label: 'Plain', props: { modelValue: '', placeholder: 'Your name' } },
      {
        label: 'With icon',
        props: { modelValue: 'shira@apple.com', placeholder: 'Email', icon: 'message' },
      },
...

  {
    ...
    title: 'Search Bar',
    description:
      'iOS-style search field -- pill shape, leading search glyph, optional voice mic. Drop it into a navigation header or a sheet.',
    ...
    isModel: 'modelValue',
    render: (k) =>
      `<liquid-search-bar :model-value="'${k.modelValue}'" placeholder="${k.placeholder}" :show-mic="${k.showMic}" @update:model-value="updateValue"></liquid-search-bar>`,
    variants: [
...
const onSearch = async (query) => {
...

  {
    ...
    title: 'Stepper',
    description:
      'Compact +/− control. Min/max/step clamp the value. Tap-and-hold support is up to you to bind.',
    ...
    isModel: 'modelValue',
    render: (k) =>
      `<liquid-stepper :model-value="${k.modelValue}" :min="${k.min}" :max="${k.max}" @update:model-value="updateValue"></liquid-stepper>`,
    variants: [
  ...
  // -----------------------------------------------------------
  {
    ...
    title: 'Card',
    description:
      'A flexible glass surface. Use `variant="glass"` over a wallpaper, `strong` over a busy page, `tinted` for promotional content, and `solid` for chrome-y UI.',
...

  {
    ...
    title: 'Badge',
    description:
      'Tiny status/label chips. Five variants: glass, accent, success, warning, danger. Optional leading icon or pulse dot.',
    component: 'LiquidBadge',
    knobs: {
      variant: {
        type: 'seg',
        options: ['glass', 'accent', 'success', 'warning', 'danger'],
        default: 'glass',
      },
      size: { type: 'seg', options: ['sm', 'md'], default: 'md' },
    ...
    render: (k) =>
      `<liquid-badge variant="${k.variant}" size="${k.size}" icon="${k.icon}" ${k.dot ? 'dot' : ''}>${k.label}</liquid-badge>`,
    variants: [
...
<LiquidBadge variant="danger" dot>Live</LiquidBadge>`,
...

  {
    ...
    title: 'Avatar',
    description:
      'User avatar with image or generated initials, status dot, and optional unread badge. Use `shape="rounded"` for app-style icons.',
    ...
    render: (k) =>
      `<liquid-avatar initials="${k.initials}" :size="${k.size}" status="${k.status}" shape="${k.shape}" badge="${k.badge}"></liquid-avatar>`,
    variants: [
...
<LiquidAvatar src="/u/shira.jpg" badge="3" />`,
...

  {
    ...
    title: 'List Row',
    description:
      'A single row in a settings list -- colored icon tile, title, subtitle, value, chevron. Stack them inside a `LiquidCard` for the classic grouped-list look.',
    component: 'LiquidListRow',
    knobs: {
      icon: {
        type: 'select',
        options: ['airplane', 'wifi', 'bluetooth', 'bell', 'shield'],
        default: 'airplane',
      },
      iconBg: {
        type: 'select',
        options: ['var(--orange)', 'var(--accent)', 'var(--green)', 'var(--red)', 'var(--purple)'],
        default: 'var(--orange)',
      },
    ...
    render: (k) =>
      `<div style="width:320px"><liquid-card variant="strong" padding="none" rounded="lg"><liquid-list-row icon="${k.icon}" icon-bg="${k.iconBg}" title="${k.title}" subtitle="${k.subtitle}" value="${k.value}" ${k.chevron ? 'chevron' : ''} :divider="false"></liquid-list-row></liquid-card></div>`,
    variants: [],
...

  {
    ...
    title: 'Progress',
    description:
      'Linear progress bar. Set `value` (0–100) for determinate, or pass `indeterminate` for the looping shimmer used during syncing.',
    ...
    render: (k) =>
      `<div style="width:280px"><liquid-progress :value="${k.value}" color="${k.color}" ${k.indeterminate ? 'indeterminate' : ''} label="${k.label}"></liquid-progress></div>`,
    variants: [
...
<LiquidProgress indeterminate label="Loading" />`,
...

  {
    ...
    title: 'Spinner',
    description:
      'A subtle circular indeterminate loader. Inherits `currentColor`, so it works against any surface.',
  ...
  // -----------------------------------------------------------
  {
    ...
    title: 'Modal',
    description:
      'A centered dialog, teleported to the body so no ancestor can clip it. `v-model` controls visibility; `@confirm` and `@cancel` fire when the user acts. `size` widens it from the iOS alert box up to full screen, and `:actions="false"` drops the built-in button bar for dialogs that carry their own.',
    component: '__inline_modal',
    knobs: {
      title: { type: 'text', default: 'Delete Recording?' },
      message: {
        type: 'text',
        default: "This recording will be removed from your library. This can't be undone.",
      },
      ...
      size: { type: 'select', options: ['alert', 'sm', 'md', 'lg', 'full'], default: 'alert' },
      actions: { type: 'bool', default: true },
...
/>`,
...

  {
    ...
    title: 'Context Menu',
    description:
      'A floating list of actions. Glass tinted, with icons, danger styling, keyboard hints, section headings and nested submenus. This is the panel on its own; pair it with Context Menu Trigger to open it on right-click.',
...
/>`,
...

  {
    ...
    description:
      'Wraps any content and raises a Context Menu at the pointer on right-click. Keeps the panel inside the window, and closes it on select, Escape, an outside press, a scroll or a window blur. Items may carry a keyboard hint, a section heading, or nested children for a submenu.',
    component: 'LiquidContextMenuTrigger',
    custom: 'menuTrigger',
    knobs: {},
    usage: `<LiquidContextMenuTrigger
  :items="[
    { label: 'Actions', heading: true },
    { key: 'edit', label: 'Edit', icon: 'edit', shortcut: 'Ctrl+E' },
    { key: 'duplicate', label: 'Duplicate', icon: 'duplicate' },
    { key: 'insert', label: 'Insert', children: [
...
>
...

  {
    ...
    title: 'Toast',
    description:
      'A transient notification that floats in from the bottom. Pair with a timer to auto-dismiss. Variants: glass, success, warning, danger.',
    ...
    knobs: {
      ...
      variant: {
        type: 'seg',
        options: ['glass', 'success', 'warning', 'danger'],
        default: 'glass',
      },
...
/>`,
...

  {
    ...
    title: 'Notification',
    description:
      'A lock-screen-style notification banner. Use for previewing how your app surfaces messages in iOS.',
    ...
    render: (k) =>
      `<div style="width:340px"><liquid-notification app-name="${k.appName}" app-icon="${k.appIcon}" title="${k.title}" message="${k.message}" time="${k.time}" :grouped="${k.grouped}"></liquid-notification></div>`,
    variants: [],
...
defineProps({
  ...
  // -----------------------------------------------------------
  {
    ...
    title: 'Tab Bar',
    description:
      'The floating bottom navigation. Active item gets an accent-tinted glass pill; supports badges for unread counts.',
    component: 'LiquidTabBar',
    custom: 'tabbar',
    knobs: {
      modelValue: {
        type: 'seg',
        options: ['today', 'games', 'apps', 'arcade'],
        default: 'today',
        label: 'active',
      },
    },
...
/>`,
  ...
  // -----------------------------------------------------------
  {
    ...
    title: 'Audio Player',
    description:
      'Mini-player card with cover art, scrubbable progress, and play/skip controls. Times tick when playing.',
    ...
    knobs: {
      ...
      coverColor: {
        type: 'select',
        options: [
          'linear-gradient(135deg,#ff6b9d,#a85ef2)',
          'linear-gradient(135deg,#ff5a5a,#ff9a3c)',
          'linear-gradient(135deg,#30d158,#0a84ff)',
          'linear-gradient(135deg,#bf5af2,#5e5ce6)',
        ],
        default: 'linear-gradient(135deg,#ff5a5a,#ff9a3c)',
      },
      duration: { type: 'slider', min: 60, max: 600, default: 240 },
      compact: { type: 'bool', default: false },
    },
    render: (k) =>
      `<liquid-audio-player title="${k.title}" artist="${k.artist}" cover-color="${k.coverColor}" :duration="${k.duration}" ${k.compact ? 'compact' : ''}></liquid-audio-player>`,
    variants: [
...
/>`,
  ...
  // -----------------------------------------------------------
  {
    ...
    title: 'Keyboard',
    description:
      'The iOS soft keyboard, rendered as glass keys. Emits a `key` event for every press -- wire it to an input or your own state machine.',
    ...
    render: (k) =>
      `<liquid-keyboard layout="${k.layout}" :caps-lock="${k.capsLock}" @key="onKey"></liquid-keyboard>`,
    variants: [],
