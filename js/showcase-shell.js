...
const { createApp, defineComponent, ref, reactive, computed, watch, onMounted, h, Transition } = Vue;
  ...
  setup(props) {
    const compiled = computed(() => {
      try {
        ...
        // Vue 3 may return a function OR { render }
        return typeof r === 'function' ? r : (r.render || (() => null));
      } catch (e) { console.warn('compile error', e); return () => null; }
    });
    return () => {
      try {
        return compiled.value.call(props.data, props.data);
      } catch (e) {
        return h('div', { style: { color: 'tomato', fontSize: '12px' } }, String(e.message || e));
      }
    };
  }
});
...
function highlight(code) {
  ...
  const KEYWORDS = new Set(['const','let','var','import','export','from','default','function','return','defineProps','defineEmits','setup','defineComponent','if','else','for','await','async']);
  ...
  while (i < n) {
    ...
    if (!inTag && c === '<') {
      ...
      if (j > start) {
        out.push('&lt;' + (code[i+1] === '/' ? '/' : '') + '<span class="tk-tag">' + escape(code.slice(start, j)) + '</span>');
    ...
    if (inTag && inAttrVal === null && c === '>') {
      out.push('&gt;'); inTag = false; i++; continue;
    }
    ...
    return () => h('div', { class: 'code-panel' }, [
      h('div', { class: 'code-head' }, [
        h('div', { class: 'code-tabs' }, [
          h('div', { class: ['code-tab', { active: tab.value === 'usage' }], onClick: () => tab.value = 'usage' }, 'Usage'),
          props.sfc
            ? h('div', { class: ['code-tab', { active: tab.value === 'sfc' }], onClick: () => tab.value = 'sfc' }, 'Component.vue')
            : null,
        ]),
      ...
      h('div', { class: 'code-body' }, [
        h('pre', { innerHTML: highlight(body.value || '') })
      ])
    ]);
  }
});
...
const PropsPlayground = defineComponent({
  ...
  setup(props, { emit }) {
    return () => {
      ...
      if (keys.length === 0) {
        return h('div', { class: 'muted', style: 'font-size:12px' }, 'No props for this component.');
      }
      return h('div', null, keys.map(k => renderKnob(k, props.knobs[k], props.values[k], (v) => emit('update', k, v))));
    };
  }
});
...
function renderKnob(name, def, val, set) {
  ...
  const valueDisplay = def.type === 'slider' ? val : (def.type === 'bool' ? (val ? 'true' : 'false') : '');
  return h('div', { class: 'knob', key: name }, [
    ...
    def.type === 'select'
      ...
      : def.type === 'seg'
      ? h('div', { class: 'knob-seg' }, def.options.map(o => h('button', {
          class: { active: o === val },
          onClick: () => set(o)
        }, String(o))))
      : def.type === 'bool'
      ? h('div', { class: 'knob-bool' },
          h('div', { onClick: () => set(!val), style: 'cursor: pointer' },
            h(window.LiquidToggle, { modelValue: val, 'onUpdate:modelValue': set, size: 'sm' })
          )
        )
      : def.type === 'slider'
      ? h('input', {
...
const MenuDemo = defineComponent({
  ...
  setup() {
    ...
    return () => h('div', null, [
      ...
      lastAction.value
        ? h('div', { style: 'margin-top:14px;font-size:12px;color:var(--text-3);text-align:center' },
  ...
  setup(props) {
    ...
    return () => h('div', null, [
      h('div', { class: 'demo-row', style: 'margin-bottom:18px' }, [
        h(window.LiquidButton, { variant: 'secondary', size: 'sm', onClick: show }, () => 'Replay')
      ]),
...
const VariantCell = defineComponent({
  ...
  setup(props) {
    ...
    watch(() => props.variant, () => { local.value = props.variant.props?.modelValue; }, { deep: false });
    return () => {
      ...
      if (hasModel) {
        ...
        propsObj['onUpdate:modelValue'] = (v) => { local.value = v; };
      }
  ...
  setup(props, { emit }) {
    return () => {
      const filtered = props.query
        ? props.pages.filter(p => (p.label + ' ' + p.title).toLowerCase().includes(props.query.toLowerCase()))
        : props.pages;
      // Group by category
      const groups = {};
      filtered.forEach(p => {
      ...
      return h('aside', { class: 'sidebar' }, [
        ...
        h('div', { class: 'side-search' }, [
          ...
          h('input', {
            ...
            value: props.query, onInput: e => emit('search', e.target.value)
          })
        ]),
        ...order.filter(g => groups[g]).map(g => h('div', { class: 'nav-group' }, [
          h('div', { class: 'nav-group-label' }, g),
          ...groups[g].map(p => h('div', {
...
const Page = defineComponent({
  ...
  setup(props) {
    ...
    const setKnob = (key, v) => { knobValues[key] = v; };
    ...
    return () => {
      ...
      else if (page.custom === 'tabbar') demoBody = h(TabBarDemo, { knobs: knobValues, onUpdate: setKnob });
      else if (typeof page.render === 'function') {
        const tpl = page.render(knobValues);
        demoBody = h(DynamicTemplate, { tpl: `<div class="demo-row">${tpl}</div>`, data: dataCtx.value });
      } else {
        demoBody = h('div', { class: 'muted' }, 'No demo');
      }

      const variants = page.variants && page.variants.length ? h('div', { class: 'section' }, [
        h('div', { class: 'section-label' }, [
          h('div', { class: 'section-title' }, 'Variants'),
          h('div', { class: 'section-meta' }, `${page.variants.length} examples`),
        ]),
        h('div', { class: 'variant-grid' }, page.variants.map(v => h(VariantCell, { page, variant: v })))
      ]) : null;
      ...
      return h('main', { class: 'main' }, [
        h('div', { class: 'main-inner' }, [
          ...
          h('div', { class: 'section' }, [
            ...
            h('div', { class: ['demo', page.id === 'keyboard' ? 'tall' : '', page.id === 'audio' ? 'tall' : ''] }, demoBody),
          ]),
          variants,
          h('div', { class: 'section' }, [
            h('div', { class: 'section-label' }, [
...
window.ShowcaseShell = { Sidebar, Page, CodePanel, PropsPlayground, DynamicTemplate, highlight,
  ModalDemo, MenuDemo, ToastDemo, TabBarDemo, VariantCell };
